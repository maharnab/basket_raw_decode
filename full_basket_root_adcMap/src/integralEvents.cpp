...
int main(int argc, char* argv[]) {
    ...
    const char* filePath = argv[1 + arg_offset];
    int minPosition_lower = atoi(argv[2 + arg_offset]);
    std::string base_dir = argv[3 + arg_offset];
    int basket_num = std::stoi(argv[4 + arg_offset]);
            ...
            while (bytesRead < totalSize && file.read(buffer.data(), bufferSize)) {    // loop over the file until the end of the file
                ...
                if (bufferSize==4){
                    ...
                    while (static_cast<std::vector<unsigned int>::size_type>(wordOffset) < (words.size() - 1)) {
                        ...
                        // Find order in adc_addresses (order = index+1)
                        int adcOrder = -1;
                        for (size_t idx = 0; idx < adc_addresses.size(); ++idx) {
                            if (device_id == adc_addresses[idx]) {
                                adcOrder = static_cast<int>(idx) + 1; // order is 1-based
                                break;
                            }
                        }
                        if (adcOrder == -1) {
                        ...
                        if (adcPayloadLength > 5){
                            ...
                            for (int j = 0; j < (adcPayloadLength-5)/waveform_words; j++) {     // loop over the adc payload
                                ...
                                if (*minElement < -100) {    // if the minimum element is less than -100
